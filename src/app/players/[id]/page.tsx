...
export default async function PlayerDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  const scoringPerf = currentScoringPerformance(p, competition?.season ?? null, now, performanceScope);
  const scoringMinutesPercent = playingTimePercent(scoringPerf);
  const playingTimeComponent = scorePlayingTime(scoringMinutesPercent);
  ...
  const perf = scoringPerf ? {
    ...
    minutesPlayedPercent: scoringMinutesPercent == null ? null : formatPercentage(scoringMinutesPercent).slice(0, -1),
  } : null;
  const sportingPerformance = selectCurrentPerformance(p.performances, performanceScope);
