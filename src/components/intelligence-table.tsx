...
export function IntelligenceTable({ columns, rows, label }: { columns: TableColumn[]; rows: TableRow[]; label: string }) {
  ...
  return <div><div className="mb-3 flex items-center justify-between text-xs text-slate-400"><span>{visible.length} / {rows.length} results</span><button type="button" onClick={() => {setFilters({});setSort(null);}} className="rounded border border-slate-600 px-3 py-2">Reset filters & sort</button></div><div className="table-wrap intelligence-table-wrap"><table aria-label={label}><thead><tr>{columns.map(column => <th key={column.key} aria-sort={sort?.key === column.key ? sort.asc ? "ascending" : "descending" : "none"}><button type="button" className="flex w-full items-center gap-2 py-1 text-left" onClick={() => setSort({key: column.key, asc: sort?.key === column.key ? !sort.asc : true})}>{column.label}<span aria-hidden="true">{sort?.key === column.key ? sort.asc ? "↑" : "↓" : "↕"}</span></button></th>)}</tr><tr>{columns.map(column => <th key={column.key}><input aria-label={`${label}: ${column.numeric ? "Minimum" : "Filter"} ${column.label}`} type={column.numeric ? "number" : "search"} step="any" value={filters[column.key] ?? ""} onChange={event => setFilters({...filters,[column.key]:event.target.value})} placeholder={column.numeric ? "Minimum…" : "Filter…"} className="w-full min-w-24 rounded border border-slate-600 bg-slate-900 px-2 py-2 text-xs normal-case tracking-normal"/></th>)}</tr></thead><tbody>{visible.map(row => <tr key={row.id}>{columns.map((column,index) => { const href=row.links?.[column.key] ?? (index === 0 ? row.href : undefined); return <td key={column.key} data-label={column.label}>{href ? <Link className="player-link" href={href}>{row.values[column.key] ?? "—"}</Link> : row.values[column.key] ?? "—"}</td>})}</tr>)}</tbody></table>{!visible.length && <p className="empty">No results match these filters.</p>}</div></div>;
}
